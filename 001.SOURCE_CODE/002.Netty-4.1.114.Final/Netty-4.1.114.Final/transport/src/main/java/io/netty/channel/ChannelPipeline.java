 ...
 *
 * Each channel has its own pipeline and it is created automatically when a new channel is created.（每个通道都有自己的管道，并且在创建新通道时会自动创建。）
 *
 * <h3>How an event flows in a pipeline（事件如何在管道中流动）</h3>
 *
 ...
 * <pre>
 *     传播方法定义在 ChannelHandlerContext 中
 * </pre>
 *
 * <pre>
 *                                                 I/O Request
 ...
 * silently, or logged if it needs your attention.(入站事件由入站处理程序按自下而上的方向处理，如图左侧所示。
 * 入站处理程序通常处理由图底部的 I/O 线程生成的入站数据。入站数据通常通过实际输入操作（如 SocketChannel.read(ByteBuffer)）从远程对等端读取。
 * 如果入站事件超出顶部入站处理程序的范围，则会被默默丢弃，或者在需要您注意时进行记录。)
 * <p>
 ...
 * {@link SocketChannel#write(ByteBuffer)}.(出站事件由出站处理程序按自上而下的方向处理，如图右侧所示。出站处理程序通常生成或转换出站流量（如写入请求）。
 * 如果出站事件超出底部出站处理程序的范围，则会由与 Channel 关联的 I/O 线程处理。
 * I/O 线程通常执行实际输出操作，如 SocketChannel.write(ByteBuffer)。)
 * <p>
 * For example, let us assume that we created the following pipeline:（例如，假设我们创建如下的pipeline）
 * <pre>
 ...
 * the evaluation of certain(肯定) handlers to shorten the stack depth:
 * <ul>
 ...
 * {@link ChannelHandlerContext} to forward an event to its next handler.  Those methods include:(传播方法有哪些)
 * <ul>
 * <li>Inbound event propagation methods: 写入响应方法
 *     <ul>
 ...
 *     </ul>
 * </li>
 * <li>Outbound event propagation methods: 写出响应方法
 *     <ul>
 ...
 * protocol and business logic:(但您的流程可能会根据协议和业务逻辑的复杂性和特点而有所不同：)
 *
 * <ol>
 ...
 * // a time-consuming task(耗时的任务). 使用另外的线程而不是I/O线程来执行MyBusinessLogicHandler的事件处理器方法
 * // If your business logic is fully asynchronous or finished very quickly, you don't
 * // need to specify a group. group 参数的含义
 * pipeline.addLast(group, "handler", new MyBusinessLogicHandler());
 * </pre>
 *
 * Be aware that while using {@link DefaultEventLoopGroup} will offload(卸载) the operation from the {@link EventLoop} it will
 * still process tasks in a serial fashion per {@link ChannelHandlerContext} and so guarantee ordering. Due the ordering
 * it may still become a bottle-neck(瓶颈). If ordering is not a requirement for your use-case you may want to consider using
 * {@link UnorderedThreadPoolEventExecutor} to maximize the parallelism of the task execution.(请注意，虽然使用DefaultEventLoopGroup将从EventLoop卸载操作，
 * 但它仍将根据ChannelHandlerContext以串行方式处理任务，从而保证排序。由于排序，它仍可能成为一个瓶颈。如果排序不是用例的要求
 * ，您可能需要考虑使用UnorderedThreadPoolEventExecutor来最大限度地提高任务执行的并行性。)
 *
 * <h3>Thread safety</h3>
 * <p>
